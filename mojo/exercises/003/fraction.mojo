...
struct Fraction(Stringable):
    ...
    fn __iadd__(inout self: Self, other: Self):
        ...
        var lcm: Int = math.lcm(self.denomirator, other.denomirator)

        # Calculate new numerators of 2 fractions 
        # (after reducing to the same denominator)
        var selfNumerator = ((lcm / self.denomirator) * self.numerator).to_int()
        var otherNumerator = ((lcm / other.denomirator) * other.numerator).to_int()

        # Calculate the numberator, denomirator of the result
        var resultNumerator: Int = selfNumerator + otherNumerator
        var resultDenomirator = lcm
        ...
        var gcd = math.gcd(resultNumerator, resultDenomirator)
